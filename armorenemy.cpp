...
const QSize ArmorEnemy::ms_fixedSize(52, 52);

ArmorEnemy::ArmorEnemy(WayPoint *startWayPoint, MainWindow *game, const QPixmap &sprite):
...
void ArmorEnemy::doActivate()
{
    m_active = true;
}

void ArmorEnemy::move()
{
    if (!m_active)
        return;
...
void ArmorEnemy::draw(QPainter *painter) const
{
    if (!m_active)
        return;
...
void ArmorEnemy::getDamage(int damage)
{
    m_game->audioPlayer()->playSound(LaserShootSound);
    //攻击量减少一半即为盔甲
    m_currentHp -= damage/2;
...
void ArmorEnemy::getAttacked(PurpleTower *attacker)
{
    m_PurpleTowerattackedTowersList.push_back(attacker);
}
void ArmorEnemy::getAttacked(BlueTower *attacker)
{
    m_BlueTowerattackedTowersList.push_back(attacker);
}
void ArmorEnemy::getAttacked(RedTower *attacker)
{
    m_RedTowerattackedTowersList.push_back(attacker);
}

// 表明敌人已经逃离了攻击范围
void ArmorEnemy::gotLostSight(PurpleTower *attacker)
{
    m_PurpleTowerattackedTowersList.removeOne(attacker);
}
void ArmorEnemy::gotLostSight(BlueTower *attacker)
{
    m_BlueTowerattackedTowersList.removeOne(attacker);
}
void ArmorEnemy::gotLostSight(RedTower *attacker)
{
    m_RedTowerattackedTowersList.removeOne(attacker);
}

QPoint ArmorEnemy::pos() const
{
    return m_pos;
}
